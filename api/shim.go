...
func prepareShim(project common.AppProject, shim string) (bool, error) {
	...
	descriptor, err := util.ParseAppDescriptor(flowJSON)
	if err != nil {
		return false, err
	}
	...
	for _, trgCfg := range descriptor.Triggers {
		if trgCfg.Id == shim {
			...
			if trgCfg.Ref != "" {
				found := false
				ref, found = GetAliasRef("AIflow:trigger", trgCfg.Ref)
				if !found {
					return false, fmt.Errorf("unable to determine ref for trigger: %s", trgCfg.Id)
				}
			}

			refImport, err := util.NewAIflowImportFromPath(ref)
			if err != nil {
				return false, err
			}
...
func createShimSupportGoFile(project common.AppProject) error {
	...
	corePath, err := project.GetPath(flowCoreImport)
	if err != nil {
		return err
	}
...
func registerImports(project common.AppProject, appDesc *util.AIflowAppDescriptor) error {
