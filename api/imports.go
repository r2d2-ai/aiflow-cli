...
func ListProjectImports(project common.AppProject) error {
	...
	appImports, err := util.GetAppImports(filepath.Join(project.Dir(), fileAIflowJson), project.DepManager(), false)
	if err != nil {
		return err
	}
...
func ResolveProjectImports(project common.AppProject) error {
	...
	if Verbose() {
		fmt.Fprintln(os.Stdout, "Reading AIflow.json")
	}
	...
	if Verbose() {
		fmt.Fprintln(os.Stdout, "Updating AIflow.json import versions")
	}
	...
	if Verbose() {
		fmt.Fprintln(os.Stdout, "Saving updated AIflow.json")
	}
...
func updateDescriptorImportVersions(project common.AppProject, appDescriptor *app.Config) error {
	...
	for _, appImport := range appImports {
		...
		if goModImport, ok := goModImports[appImport.ModulePath()]; ok {
			updatedImp := util.NewAIflowImportWithVersion(appImport, goModImport.Version())
