...
func readAppDescriptor(project common.AppProject) (*app.Config, error) {
	...
	appDescriptorFile, err := os.Open(filepath.Join(project.Dir(), fileAIflowJson))
	if err != nil {
		return nil, err
	}
...
func writeAppDescriptor(project common.AppProject, appDescriptor *app.Config) error {
	...
	err = ioutil.WriteFile(filepath.Join(project.Dir(), fileAIflowJson), []byte(appDescriptorUpdatedJson), 0644)
	if err != nil {
		return err
	}
