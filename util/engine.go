...
// PartialEngineDescriptor is the descriptor for a AIflow application
...
func (ai *EngineImports) addImports(imports []string) error {
	for _, anImport := range imports {
		flowImport, err := ParseImport(anImport)
		if err != nil {
			return err
		}

		if _, exists := ai.imports[flowImport.GoImportPath()]; exists {
			//todo warn about duplicate import?
			continue
		}

		details := &EngineImportDetails{Imp: flowImport, TopLevel: true}
		ai.imports[flowImport.GoImportPath()] = details
	}
...
func (ai *EngineImports) addReference(ref string, isService bool) error {
	...
	if cleanedRef[0] == '#' {
		...
		flowImport, err := ParseImport(ref)
		if err != nil {
			return err
		}

		if imp, exists := ai.imports[flowImport.GoImportPath()]; exists {
			if !imp.TopLevel {
		...
		details := &EngineImportDetails{Imp: flowImport, ServiceRef: isService}
		ai.imports[flowImport.GoImportPath()] = details
	}
...
func GetEngineImports(engJsonFile string, depManager DepManager) (*EngineImports, error) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "Unable to unmarshal AIflow engine json: %s", engJsonFile)
		return nil, err
	}
