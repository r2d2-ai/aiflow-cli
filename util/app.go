...
// PartialAppDescriptor is the descriptor for a AIflow application
...
type AppImportDetails struct {
	Imp          Import
	ContribDesc  *AIflowContribDescriptor
...
func (ai *AppImports) addImports(imports []string) error {
	for _, anImport := range imports {
		...
		details, err := ai.newImportDetails(flowImport)
		if err != nil {
			return err
		}
...
func GetAppImports(appJsonFile string, depManager DepManager, resolveContribs bool) (*AppImports, error) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "Unable to unmarshal AIflow app json: %s", appJsonFile)
		return nil, err
	}
